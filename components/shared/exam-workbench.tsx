...
export function ExamWorkbench({ questions, canPersist = false }: ExamWorkbenchProps) {
  ...
  async function handleSave() {
    ...
    try {
      /*
        Dal `createExamFromPool` cagiriyordu; develop'ta o eylemin yerini
        `createExamWithQuestions` aldi ve daha genis: ders, sure ve kamera
        zorunlulugu da alabiliyor. Iki fark uyarlandi:
          - puanlar dizi degil, soru kimligi -> puan haritasi,
          - `date` alani yok; tarih sinav ayarlarindan yonetiliyor, o yuzden
            kagit basligindaki tarih yalnizca ciktida gorunuyor.
      */
      const result = await createExamWithQuestions({
        ...
        points: Object.fromEntries(
          paperQuestions.map((question) => [question.id, question.points]),
        ),
      });
